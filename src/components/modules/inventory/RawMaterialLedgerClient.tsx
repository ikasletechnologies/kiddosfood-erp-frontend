...
export default function RawMaterialLedgerClient() {
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in duration-500 p-4 md:p-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-4 bg-slate-50 dark:bg-white/5 p-3.5 sm:p-4 rounded-2xl sm:rounded-[2rem] border border-slate-100 dark:border-white/5 w-full min-w-0">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full lg:w-auto">
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <select
              ...
              className="px-3 sm:px-4 py-2 sm:py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold outline-none cursor-pointer w-full sm:w-44"
            >
          ...
          <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
            ...
            <select
              ...
              className="px-3 sm:px-4 py-2 sm:py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold outline-none cursor-pointer w-full sm:w-80"
            >
        ...
        <div className="relative group w-full lg:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-8 py-2.5 sm:py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-xl outline-none text-xs font-bold shadow-sm"
          />
          {search && (
            <X 
              size={14} 
      ...
      {selectedItemInfo && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 bg-slate-900 text-white p-4 sm:p-8 rounded-2xl sm:rounded-[2rem] shadow-xl border border-slate-800 animate-in fade-in duration-300 w-full min-w-0">
          <div className="space-y-1.5 sm:space-y-2 border-b border-slate-800 sm:border-none pb-3 sm:pb-0">
            <p className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Material Spec</p>
            <h3 className="text-base sm:text-lg font-black uppercase tracking-tight truncate">{selectedItemInfo.name}</h3>
          ...
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <h3 className="text-2xl sm:text-3xl font-black text-emerald-400 tracking-tight">
              ...
              <span className="text-xs sm:text-sm font-bold text-white ml-1.5 uppercase">{selectedItemInfo.unit}</span>
            </h3>
            <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-wider">Validated via movement ledger</p>
          </div>
          <div className="space-y-1.5 sm:space-y-2">
            <p className="text-[10px] font-black tracking-widest text-slate-400 uppercase">Unit Cost Value</p>
            <h3 className="text-2xl sm:text-3xl font-black text-blue-400 tracking-tight">
              ₹{(closingStock * selectedItemInfo.costPrice).toLocaleString(undefined, { maximumFractionDigits: 0 })}
            </h3>
            <p className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-wider">Cost Price: ₹{selectedItemInfo.costPrice.toFixed(2)}</p>
          </div>
        </div>
      )}

      {/* Ledger Table */}
      <div className="bg-white dark:bg-card/40 border border-slate-200 dark:border-white/5 rounded-2xl sm:rounded-[2.5rem] shadow-xl overflow-hidden w-full min-w-0">
