...
export default function LabelsBarcodesPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-background text-gray-800 dark:text-slate-100">
      {/* Page Header Toolbar */}
      <div className="bg-white dark:bg-card border-b border-gray-200 dark:border-white/5 px-6 py-3 flex flex-col md:flex-row md:items-center justify-end gap-3 print:hidden">
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-3">
                              <div className="font-medium text-gray-800 dark:text-white">{rec.batch?.product?.name}</div>
                              <div className="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Batch: {rec.batch?.batchCode}</div>
                            </td>
                            ...
                            <td className="px-4 py-3 text-center">
                              <button
                                className="p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-slate-200 hover:bg-gray-100 dark:hover:bg-white/5 rounded-lg transition-colors"
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setSelectedRecord(rec);
                                  resetForm(rec.quantityPackets);
          ...
          <div className="lg:col-span-1">
            {selectedRecord ? (
              <div className="space-y-5">
                ...
                {/* Action controller */}
                <div className="bg-white dark:bg-card rounded-lg border border-gray-200 dark:border-white/5 p-4 space-y-3 print:hidden shadow-sm">
                  <h4 className="text-xs font-semibold text-gray-700 dark:text-slate-200 uppercase tracking-wide">Labeling Actions</h4>
              ...
              <div className="hidden lg:flex flex-col items-center justify-center py-24 border border-dashed border-gray-200 dark:border-white/10 rounded-lg text-center p-6 bg-white dark:bg-card print:hidden">
                <Barcode className="h-8 w-8 text-gray-300 dark:text-slate-600 mb-3" />
                <p className="text-sm text-gray-400 dark:text-slate-500">Select a record to preview compliance label</p>
              </div>
            )}
          </div>
