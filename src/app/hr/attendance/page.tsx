        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-400">No employees found</td></tr>
            ) : filtered.map((emp) => {
              const log = todayLogs[emp.id];
              return (
                <tr key={emp.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-900">{emp.user?.fullName}</p>
                    <p className="text-xs text-gray-400">{emp.employeeCode}</p>
                  </td>
                  <td className="px-4 py-3 text-gray-700">{fmtTime(log?.clockIn) || "—"}</td>
                  <td className="px-4 py-3 text-gray-700">{fmtTime(log?.clockOut) || "—"}</td>
                  <td className="px-4 py-3">
                    {!log?.clockIn ? (
                      <button
                      ...
                      <span className="text-xs font-medium text-emerald-600">Day Complete</span>
                    )}
                  </td>
                </tr>
              );
