    ...
    <div className="max-w-5xl mx-auto space-y-8 py-4 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-6 border-b border-gray-100 pb-8">
        <div className="space-y-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-black text-slate-900 tracking-tight">Trial Balance</h1>
          </div>
      ...
      {loading ? (
        <div className="py-40 flex flex-col items-center justify-center gap-6">
          ...
          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Loading Trial Balance</p>
        </div>
        ...
        <div className="py-32 flex flex-col items-center justify-center gap-3 text-center">
          ...
          <p className="text-sm font-semibold text-slate-500">No ledger activity for the selected period.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-gray-100">
                <th className="text-left px-6 py-3.5 font-bold text-slate-500 uppercase text-[11px] tracking-wider">Particulars</th>
                <th className="text-right px-6 py-3.5 font-bold text-slate-500 uppercase text-[11px] tracking-wider">Debit</th>
                <th className="text-right px-6 py-3.5 font-bold text-slate-500 uppercase text-[11px] tracking-wider">Credit</th>
              </tr>
            </thead>
            <tbody>
              {nonZeroRows.map((row, idx) => (
                <tr key={idx} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-6 py-3 text-right font-mono text-slate-700">{row.debit ? formatAmount(row.debit) : "—"}</td>
                  <td className="px-6 py-3 text-right font-mono text-slate-700">{row.credit ? formatAmount(row.credit) : "—"}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="bg-slate-50 border-t-2 border-slate-200">
                ...
                <td className="px-6 py-4 text-right font-black font-mono text-slate-900">{formatAmount(totalDebit)}</td>
                <td className="px-6 py-4 text-right font-black font-mono text-slate-900">{formatAmount(totalCredit)}</td>
              </tr>
            </tfoot>
          </table>
          {Math.abs(totalDebit - totalCredit) > 0.01 && (
            <div className="px-6 py-3 bg-amber-50 border-t border-amber-100 text-xs font-semibold text-amber-700">
