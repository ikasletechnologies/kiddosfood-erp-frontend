...
export default function GSTR1Page() {
  ...
  const [tab, setTab] = useState<"sale" | "saleReturn">("sale");
  ...
  // Normalize all sales returns from approved/completed ReturnOrders and refunded POS orders
  const salesReturns: ReturnRow[] = useMemo(() => {
    const list: ReturnRow[] = [];
    const seen = new Set<string>();

    (data.creditNotes || []).forEach((r: any) => {
      const num = r.invoiceNo || r.returnNumber || "";
      if (num && !seen.has(num)) {
        seen.add(num);
        list.push({
          invoiceNo: num,
          date: r.date || r.createdAt?.split("T")[0] || "—",
          partyName: r.partyName || r.customer?.name || "Cash Customer",
          gstin: r.gstin || r.customer?.gstNumber || "—",
          placeOfSupply: r.placeOfSupply || "—",
          taxRate: r.taxRate,
          taxableValue: Math.abs(r.taxableValue || 0),
          cgst: Math.abs(r.cgst || 0),
          sgst: Math.abs(r.sgst || 0),
          igst: Math.abs(r.igst || 0),
          cessAmount: Math.abs(r.cessAmount || 0),
          totalTax: Math.abs(r.totalTax || r.taxAmount || 0),
          totalAmount: Math.abs(
            r.totalAmount ||
              r.refundAmount ||
              Math.abs(r.taxableValue || 0) + Math.abs(r.totalTax || r.taxAmount || 0)
          ),
        });
      }
    });

    (data.saleReturn || []).forEach((r: GSTR1Row) => {
      ...
      if (num && !seen.has(num)) {
        ...
        list.push({
          ...
          igst: Math.abs(r.igst || 0),
          cessAmount: Math.abs(r.cessAmount || 0),
          totalTax: Math.abs(r.totalTax || 0),
          totalAmount: Math.abs(r.totalAmount || Math.abs(r.taxableValue || 0) + Math.abs(r.totalTax || 0)),
        });
      }
    });

    return list;
  }, [data.creditNotes, data.saleReturn]);

  const rates = useMemo(
    () => Array.from(new Set(data.sale.map((r) => r.taxRate))).filter((r) => r != null && !isNaN(r)).sort((a, b) => a - b),
    [data.sale]
  );

  const matchesSearch = (r: { partyName?: string; invoiceNo?: string; gstin?: string; placeOfSupply?: string }) => {
  ...
  const filteredReturns = useMemo(() => salesReturns.filter(matchesSearch), [salesReturns, search]);

  const current = tab === "sale" ? filteredSale : filteredReturns;

  const fmt = (n: number) => `₹${(n || 0).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

  const headers =
    tab === "sale"
      ? ["Invoice No", "Date", "Party", "GSTIN", "Type", "Place of Supply", "Tax Rate", "Taxable Value", "CGST", "SGST", "IGST", "Cess", "Total Tax", "Invoice Value"]
      : ["Return / Invoice No", "Date", "Party", "GSTIN", "Place of Supply", "Tax Rate", "Taxable Value", "CGST", "SGST", "IGST", "Cess", "Total Tax", "Refund Value"];

  const toRows = (rows: any[]) =>
    rows.map((r: any) =>
      tab === "sale"
        ? [r.invoiceNo, r.date, r.partyName, r.gstin, r.b2bType || "B2C", r.placeOfSupply || "—", r.taxRate != null ? `${r.taxRate}%` : "—", r.taxableValue, r.cgst, r.sgst, r.igst, r.cessAmount ?? 0, r.totalTax, r.totalAmount ?? ""]
        : [r.invoiceNo, r.date, r.partyName, r.gstin, r.placeOfSupply || "—", r.taxRate != null ? `${r.taxRate}%` : "—", r.taxableValue, r.cgst, r.sgst, r.igst, r.cessAmount ?? 0, r.totalTax, r.totalAmount ?? ""]
    );

  const filenameBase = `GSTR1_${tab === "sale" ? "Sales" : "Sales_Return"}_${appliedRange.startDate}_${appliedRange.endDate}`;
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-white/10 rounded-2xl p-3 sm:p-4 shadow-2xs print:hidden">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Search invoice no, party name, GSTIN, place of supply…"
              value={search}
              ...
              <button
                onClick={() =>
                  exportPdf(
                    `${filenameBase}.pdf`,
                    "GSTR-1 Outward Supplies",
                    `${tab === "sale" ? "Sales" : "Sales Return"} · ${appliedRange.startDate} to ${appliedRange.endDate}`,
                    headers,
                    toRows(current)
                  )
                }
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xs overflow-hidden print:border-none print:shadow-none">
        ...
        <div className="p-3.5 sm:p-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between gap-3 print:hidden">
          <div className="flex items-center bg-gray-100 dark:bg-white/5 p-1 rounded-xl border border-gray-200 dark:border-white/10 shrink-0">
            ...
            >
              Sale Invoices ({data.sale.length})
            </button>
            <button
              onClick={() => setTab("saleReturn")}
              className={clsx(
                "px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer",
                tab === "saleReturn"
                  ? "bg-white dark:bg-card text-[#f58220] shadow-2xs"
            ...
            >
              Sales Return ({salesReturns.length})
            </button>
          </div>
        ...
        <div className="overflow-x-auto custom-scrollbar w-full min-w-0 print:overflow-visible">
          {loading ? (
            <div className="py-24 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs font-medium text-gray-500 dark:text-slate-400">Loading GSTR-1 records…</p>
            </div>
          ) : current.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center gap-2.5 text-center px-4">
              ...
              <p className="text-sm font-bold text-gray-700 dark:text-slate-200">
                No {tab === "sale" ? "sales invoices" : "sales returns"} found
              </p>
            ...
            <table className="w-full text-xs text-left border-collapse min-w-[900px] print:min-w-0 print:text-[9pt]">
              <thead>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-white/5 print:divide-gray-200">
                {current.map((r: any, idx) => (
                  <tr key={idx} className="hover:bg-gray-50/70 dark:hover:bg-white/[0.02] transition-colors print:hover:bg-transparent">
                    <td className="px-3.5 py-2.5 font-bold text-[#f58220] whitespace-nowrap print:text-black print:px-2 print:py-1.5">{r.invoiceNo}</td>
                    ...
                    <td className="px-3.5 py-2.5 font-semibold text-gray-900 dark:text-white max-w-[180px] truncate print:text-black print:px-2 print:py-1.5">{r.partyName}</td>
                    <td className="px-3.5 py-2.5 text-gray-600 dark:text-slate-300 font-mono text-[11px] whitespace-nowrap print:text-black print:px-2 print:py-1.5">{r.gstin}</td>
                    {tab === "sale" && (
                      <td className="px-3.5 py-2.5 whitespace-nowrap print:px-2 print:py-1.5">
                        <span className={clsx("px-2 py-0.5 rounded-md text-[10px] font-bold border print:border-none print:p-0", r.b2bType === "B2B" ? "bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-500/10 dark:text-purple-400 dark:border-purple-500/20" : "bg-gray-50 text-gray-600 border-gray-200 dark:bg-white/5 dark:text-slate-400 dark:border-white/10")}>
                          {r.b2bType || "B2C"}
                        </span>
                      </td>
                    )}
                    ...
                    <td className="px-3.5 py-2.5 text-right text-gray-600 dark:text-slate-300 whitespace-nowrap print:text-black print:px-2 print:py-1.5">{r.taxRate != null ? `${r.taxRate}%` : "—"}</td>
                    ...
                    <td className="px-3.5 py-2.5 text-right font-mono font-bold text-[#f58220] whitespace-nowrap print:text-black print:px-2 print:py-1.5">{r.totalAmount != null ? fmt(r.totalAmount) : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
