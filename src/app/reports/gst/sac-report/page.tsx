...
export default function SacReportPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await reportsApi.getSacReport({
    ...
    () =>
      rows.filter((r) => {
        ...
        return (
          ...
          (r.sacCode || "").toLowerCase().includes(q)
        );
      }),
    [rows, search]
  );
  ...
  const toRows = filtered.map((r) => [r.sacCode, r.serviceName, r.gstRate != null ? `${r.gstRate}%` : "—", r.taxableValue, r.cgst, r.sgst, r.igst, r.totalTax]);
  const filenameBase = `SACReport_${appliedRange.startDate}_${appliedRange.endDate}`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-4 sm:space-y-5 p-3.5 sm:p-6 text-slate-800 dark:text-slate-100 w-full min-w-0 print:p-0 print:m-0 print:max-w-none">
      ...
      <div className="hidden print:flex flex-row justify-between items-start border-b-2 border-gray-800 pb-3 mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">SAC REPORT (SERVICES SUMMARY)</h1>
      ...
      <div className="bg-white dark:bg-card border border-gray-200 dark:border-white/10 rounded-2xl p-3 sm:p-4 shadow-2xs print:hidden">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              type="text"
              placeholder="Search by service name or SAC code…"
              value={search}
        ...
        <div className="overflow-x-auto custom-scrollbar w-full min-w-0 print:overflow-visible">
          {loading ? (
            <div className="py-24 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs font-medium text-gray-500 dark:text-slate-400">Loading SAC report records…</p>
            </div>
          ) : filtered.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center gap-2.5 text-center px-4">
              ...
              <p className="text-sm font-bold text-gray-700 dark:text-slate-200">No service sales found</p>
              <p className="text-xs text-gray-500 dark:text-slate-400 max-w-sm">
                {search
                  ? `No service items matching "${search}" in this period.`
                  : "No services matching the selected criteria have been billed in this period."}
              </p>
              {isFiltered && (
                <button
                  onClick={handleResetFilters}
            ...
            <table className="w-full text-xs text-left border-collapse min-w-[700px] print:min-w-0 print:text-[9pt]">
              <thead>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-white/5 print:divide-gray-200">
                {filtered.map((r, idx) => (
                  <tr key={idx} className="hover:bg-gray-50/70 dark:hover:bg-white/[0.02] transition-colors print:hover:bg-transparent">
                    <td className="px-3.5 py-2.5 font-bold text-[#f58220] whitespace-nowrap print:text-black print:px-2 print:py-1.5">{r.sacCode || "—"}</td>
                    <td className="px-3.5 py-2.5 font-semibold text-gray-900 dark:text-white max-w-[200px] truncate print:text-black print:px-2 print:py-1.5">{r.serviceName}</td>
                    ...
                    <td className="px-3.5 py-2.5 text-right font-mono font-bold text-[#f58220] whitespace-nowrap print:text-black print:px-2 print:py-1.5">{fmt(r.totalTax)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
