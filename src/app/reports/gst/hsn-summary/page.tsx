...
import { RefreshCw, FileText, FileSpreadsheet, Printer, Package } from "lucide-react";
...
export default function HsnSummaryPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await reportsApi.getHsnSummary({ startDate, endDate, gstRate: gstRate || undefined });
  ...
  const filtered = useMemo(
    () => rows.filter((r) => !search || r.productName?.toLowerCase().includes(search.toLowerCase()) || r.hsn?.toLowerCase().includes(search.toLowerCase())),
    [rows, search]
  );
  ...
  const toRows = filtered.map((r) => [r.hsn, r.productName, r.unit, r.quantity, r.gstRate, r.taxableValue, r.cgstAmount, r.sgstAmount, r.igstAmount, r.totalValue]);
  const filenameBase = `SalesSummaryHSN_${startDate}_${endDate}`;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-4 sm:p-6 py-4 text-slate-800 dark:text-slate-100 w-full min-w-0">
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 border-b border-gray-100 dark:border-white/5 pb-6 w-full min-w-0">
        <div className="space-y-2 min-w-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl sm:text-4xl font-black text-slate-900 dark:text-white tracking-tight">Sales Summary (HSN)</h1>
          </div>
          <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">
            Goods sales grouped by HSN code. Service lines never appear here.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-white dark:bg-card border border-gray-100 dark:border-white/5 rounded-2xl p-4">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total GST</p>
          <p className="text-lg font-black text-slate-900 dark:text-white mt-1">{fmt(totals.cgstAmount + totals.sgstAmount + totals.igstAmount)}</p>
        </div>
        <div className="bg-white dark:bg-card border border-gray-100 dark:border-white/5 rounded-2xl p-4">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Value</p>
          <p className="text-lg font-black text-slate-900 dark:text-white mt-1">{fmt(totals.totalValue)}</p>
        </div>
        <div className="bg-white dark:bg-card border border-gray-100 dark:border-white/5 rounded-2xl p-4">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">HSN Lines</p>
      ...
      <div className="bg-white dark:bg-card border border-gray-100 dark:border-white/5 rounded-2xl shadow-sm overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-4 border-b border-gray-100 dark:border-white/5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={() => exportExcel(`${filenameBase}.xlsx`, "HSN Summary", `Sales Summary (HSN) ${startDate} to ${endDate}`, headers, toRows)} className="p-2.5 rounded-xl bg-slate-50 dark:bg-white/5 text-slate-500 hover:text-emerald-600 transition-colors" title="Export Excel"><FileSpreadsheet size={16} /></button>
            <button onClick={() => exportPdf(`${filenameBase}.pdf`, "Sales Summary (HSN)", `${startDate} to ${endDate}`, headers, toRows)} className="p-2.5 rounded-xl bg-slate-50 dark:bg-white/5 text-slate-500 hover:text-red-600 transition-colors" title="Export PDF"><Printer size={16} /></button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          {loading ? (
            ...
            <div className="py-32 flex flex-col items-center justify-center gap-3 text-center">
              ...
              <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">No goods sales for the selected period.</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                {filtered.map((r, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3 font-semibold text-blue-600">{r.hsn}</td>
                    <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-200">{r.productName}</td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{r.unit}</td>
                    ...
                    <td className="px-4 py-3 text-right font-mono font-semibold">{fmt(r.totalValue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
