...
export default function TDSPayableReport({
  reportData,
  ...
  const [data, setData] = useState<TdsPayableRow[]>([]);
  const [totalPurchaseWithTds, setTotalPurchaseWithTds] = useState(0);
  const [totalTds, setTotalTds] = useState(0);
  const [fetching, setFetching] = useState(false);

  // Sorting
  const [sortField, setSortField] = useState<keyof TdsPayableRow>("partyName");
  ...
  const fetchTds = () => {
    setFetching(true);
    reportsApi
      .getTdsPayable({ startDate, endDate })
      .then((res: any) => {
        setData(res.data?.data || []);
        setTotalPurchaseWithTds(res.data?.totalPurchaseWithTds || 0);
        setTotalTds(res.data?.totalTds || 0);
      })
      .catch(() => {
        toast.error("Failed to load TDS Payable report");
      })
      .finally(() => {
        setFetching(false);
      });
  ...
  const toggleSort = (field: keyof TdsPayableRow) => {
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
      setSortField(field);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="hidden print:block border-b-2 border-slate-300 pb-3 mb-4">
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">TDS Payable Report</h2>
      ...
      <div className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
              ...
              {loading ? (
                <tr>
                  <td colSpan={10} className="text-center py-24">
                    ...
                    <p className="text-xs text-slate-400 mt-3 font-medium">Fetching TDS Payable records...</p>
                  </td>
                </tr>
              ) : sortedData.length === 0 ? (
                <tr>
                  <td colSpan={10} className="text-center py-32 text-slate-400 text-xs font-semibold">
                    ...
                    <h4 className="text-slate-800 dark:text-slate-200 font-bold mb-1">No Data Available!</h4>
                ...
                sortedData.map((r, idx) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-[13px] font-semibold text-slate-705 dark:text-slate-450 text-right tabular-nums">
                      {r.rate} %
                    </td>
                  </tr>
                ))
        ...
        {!loading && (
          <div className="bg-slate-50 dark:bg-slate-800/40 border-t border-slate-200 dark:border-slate-700 px-5 py-4 flex items-center justify-between text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wide">
            <div>
              Total Purchase With TDS: <span className="text-[14px] text-slate-800 dark:text-slate-200 ml-1">{fmt(totalPurchaseWithTds)}</span>
            </div>
            <div>
