...
export default function CentralPartyReportByItem({
  reportData,
  ...
  // Real data state
  const [rows, setRows] = useState<PartyItemRow[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    setLoading(true);
    reportsApi.getPartyByItem({
      startDate,
      endDate
    }).then((res: any) => {
  ...
  const totalSaleQty = filtered.reduce((s, r) => s + r.saleQty, 0);
  const totalSaleAmt = filtered.reduce((s, r) => s + r.saleAmount, 0);
  const totalPurchaseQty = filtered.reduce((s, r) => s + r.purchaseQty, 0);
  ...
  return (
    <div className="flex flex-col h-full bg-[#f1f5f9] dark:bg-[#090a0f] space-y-4 p-6 overflow-hidden">
      {/* Top Header Section */}
      <div className="flex flex-col gap-4 bg-white dark:bg-[#12141c] p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 shrink-0">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="flex flex-wrap items-center gap-6">
            ...
            <div className="relative">
              <select className="px-3 pr-8 py-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-semibold text-slate-700 dark:text-slate-300 bg-transparent focus:outline-none focus:ring-1 focus:ring-blue-500 appearance-none min-w-[120px]">
      ...
      <div className="bg-white dark:bg-[#12141c] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col flex-1 overflow-hidden relative">
        ...
        <div className="overflow-auto flex-1">
          <table className="w-full text-left border-collapse min-w-[1000px]">
              ...
              <tr>
                ...
                <th className="px-4 py-3 border-r border-slate-100 dark:border-slate-800 text-center">
                  <div className="flex items-center justify-center group cursor-pointer gap-2">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">SALE QUANTITY</span>
                    <FilterIcon size={12} className="text-slate-300 group-hover:text-slate-500 transition-colors" />
                  </div>
                </th>
                ...
                <th className="px-4 py-3 border-r border-slate-100 dark:border-slate-800 text-center">
                  <div className="flex items-center justify-center group cursor-pointer gap-2">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">PURCHASE QUANTITY</span>
                    <FilterIcon size={12} className="text-slate-300 group-hover:text-slate-500 transition-colors" />
                  </div>
                </th>
            ...
            <tbody className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              {loading || externalLoading ? (
                <tr>
                  <td colSpan={6} className="px-4 py-32 text-center h-full">
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-40 text-center h-full align-middle">
                ...
                filtered.map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors bg-blue-50/30 dark:bg-blue-900/10">
                    ...
                    <td className="px-4 py-4 border-r border-slate-100 dark:border-slate-800 text-center">{row.saleQty}</td>
                    <td className="px-4 py-4 border-r border-slate-100 dark:border-slate-800 text-right">{fmt(row.saleAmount)}</td>
                    <td className="px-4 py-4 border-r border-slate-100 dark:border-slate-800 text-center">{row.purchaseQty}</td>
        ...
        {/* Footer Aligned with columns */}
        <div className="bg-white dark:bg-[#12141c] border-t border-slate-200 dark:border-slate-800 shrink-0">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <tbody>
              <tr>
                <td className="px-4 py-4 w-12 border-r border-transparent"></td>
                <td className="px-4 py-4 border-r border-transparent text-[11px] font-semibold text-slate-600 dark:text-slate-400 text-right">Total:</td>
                <td className="px-4 py-4 border-r border-transparent text-[11px] font-semibold text-slate-800 dark:text-white text-center w-[15%]">{totalSaleQty}</td>
                <td className="px-4 py-4 border-r border-transparent text-[11px] font-semibold text-slate-800 dark:text-white text-right w-[15%]">{fmt(totalSaleAmt)}</td>
                <td className="px-4 py-4 border-r border-transparent text-[11px] font-semibold text-slate-800 dark:text-white text-center w-[15%]">{totalPurchaseQty}</td>
                <td className="px-4 py-4 text-[11px] font-semibold text-slate-800 dark:text-white text-right w-[15%]">{fmt(totalPurchaseAmt)}</td>
              </tr>
            </tbody>
          </table>
        </div>
