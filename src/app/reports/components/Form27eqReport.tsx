...
export default function Form27eqReport({
  ...
  const [data, setData] = useState<Form27eqRow[]>([]);
  const [totalSaleWithTcs, setTotalSaleWithTcs] = useState(0);
  const [totalTcs, setTotalTcs] = useState(0);
  const [fetching, setFetching] = useState(false);

  // Sorting
  const [sortField, setSortField] = useState<keyof Form27eqRow>("partyName");
  ...
  const fetchReport = () => {
    setFetching(true);
    reportsApi
      .getForm27eq({ startDate, endDate })
      .then((res: any) => {
        setData(res.data?.data || []);
        setTotalSaleWithTcs(res.data?.totalSaleWithTcs || 0);
        setTotalTcs(res.data?.totalTcs || 0);
      })
      .catch(() => {
        toast.error("Failed to load Form No. 27EQ report");
      })
      .finally(() => {
        setFetching(false);
      });
  ...
  const toggleSort = (field: keyof Form27eqRow) => {
    if (sortField === field) {
      setSortAsc(!sortAsc);
    } else {
      setSortField(field);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 space-y-4 no-print shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Date Inputs */}
            <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-330">
      ...
      <div className="hidden print:block border-b-2 border-slate-300 pb-3 mb-4">
        <h2 className="text-xl font-black text-slate-900 uppercase tracking-tight">Form No. 27EQ Report</h2>
      ...
      <div className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden flex flex-col min-h-[400px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/40 select-none">
                ...
                >
                  <div className="flex items-center justify-end gap-1">
                    <span>Total Tax Collected (TCS)</span>
                    <SortIcon size={10} className="text-slate-405" />
                  </div>
                </th>
                <th
              ...
              {loading ? (
                <tr>
                  <td colSpan={8} className="text-center py-24">
                    ...
                    <p className="text-xs text-slate-400 mt-3 font-medium">Fetching Form No. 27EQ records...</p>
                  </td>
                </tr>
              ) : sortedData.length === 0 ? (
                <tr>
                  <td colSpan={8} className="text-center py-32 text-slate-400 text-xs font-semibold">
                    ...
                    <h4 className="text-slate-800 dark:text-slate-200 font-bold mb-1">No data is available for Form No. 27EQ.</h4>
        ...
        {!loading && (
          <div className="bg-slate-50 dark:bg-slate-800/40 border-t border-slate-200 dark:border-slate-700 px-5 py-4 flex items-center justify-between text-xs font-bold text-red-650 dark:text-red-400 uppercase tracking-wide">
            <div>
              Total Sale With TCS: <span className="text-[14px] text-slate-800 dark:text-slate-200 ml-1">{fmt(totalSaleWithTcs)}</span>
            </div>
            <div>
