...
export default function CentralSalePurchaseByParty({
  ...
  // Real data state
  const [rows, setRows] = useState<SalePurchaseRow[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    setLoading(true);
    reportsApi.getSalePurchaseByParty({
      startDate,
      endDate
    }).then((res: any) => {
  ...
  const fmtZero = (val: number) => `₹ ${val.toFixed(2)}`;
      ...
      <div className="bg-white dark:bg-[#12141c] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col flex-1 overflow-hidden relative">
        ...
        <div className="overflow-auto flex-1">
          <table className="w-full text-left border-collapse min-w-[1000px]">
            ...
            <tbody className="text-xs font-semibold text-slate-600 dark:text-slate-400">
              {loading || externalLoading ? (
                <tr>
                  <td colSpan={4} className="px-4 py-32 text-center h-full">
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-40 text-center h-full align-middle">
        ...
        <div className="bg-white dark:bg-[#12141c] border-t border-slate-200 dark:border-slate-800 p-4 shrink-0 flex justify-between items-center text-[11px] font-semibold tracking-wide">
          <div className="text-slate-600 dark:text-slate-400">
            Total Sale Amount: <span className="text-emerald-500 ml-1">{fmtZero(totalSaleAmt)}</span>
          </div>
          <div className="text-slate-600 dark:text-slate-400">
            Total Purchase Amount: <span className="text-red-500 ml-1">{fmtZero(totalPurchaseAmt)}</span>
          </div>
        </div>
      </div>
    </div>
